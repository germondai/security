...
export function jwtSign(opts: JwtSignOptions): string {
  ...
  const header: JwtHeaderObj = { alg: "HS256", typ: "JWT", ...opts.header };
  const payload: JwtPayloadObj = { ...opts.payload };
...
export function jwtVerify(opts: JwtVerifyOptions): JwtVerifyResult {
  ...
  let header: JwtHeaderObj, payload: JwtPayloadObj;
  try {
    header = JSON.parse(new TextDecoder().decode(fromBase64Url(h))) as JwtHeaderObj;
    payload = JSON.parse(new TextDecoder().decode(fromBase64Url(p))) as JwtPayloadObj;
  } catch {
    return { valid: false, error: "invalid json" };
  }
  if (header.alg !== "HS256")
    return { valid: false, error: `unsupported alg: ${String(header.alg)}` };
  const now = Math.floor(Date.now() / 1000);
  ...
  if (typeof payload.nbf === "number" && now < payload.nbf - skew)
    return { valid: false, header, payload, error: "token not yet valid" };
  if (opts.issuer && payload.iss !== opts.issuer)
    return { valid: false, header, payload, error: "issuer mismatch" };
  if (opts.audience && payload.aud !== opts.audience)
    return { valid: false, header, payload, error: "audience mismatch" };
