import { shannonEntropy, naiveEntropyBits } from './entropy.js';
import { detectPatterns, type DetectedPattern } from './patterns.js';
import { estimateCrackTimes, estimateCrackTimesFromBits, type CrackTimeEstimate } from './cracktime.js';
...
function effectiveEntropyBits(s: string, patterns: DetectedPattern[], poolSize: number): number {
  ...
  const distinctWords = new Set(patterns.filter(p => p.type === 'common').map(p => p.match.toLowerCase()));
  const dictionaryBits = distinctWords.size * DICT_LOG;
  const patternBits = patterns.filter(p => p.type === 'sequence' || p.type === 'repeat' || p.type === 'date').length * 5;
...
function scoreFromBits(bits: number, length: number, wordCount: number): { score: 0|1|2|3|4; percent: number; label: StrengthResult['label'] } {
  let s: 0|1|2|3|4;
...
export function analyzePassword(s: string): StrengthResult {
  ...
  if (wordCount > 0) feedback.push(`Contains ${wordCount} common word${wordCount > 1 ? 's' : ''} — concatenating dictionary words is far weaker than it looks.`);
  if (patterns.some(p => p.type === 'sequence')) feedback.push('Avoid sequential characters (abcd, 1234, qwerty).');
  if (patterns.some(p => p.type === 'repeat')) feedback.push('Avoid long runs of the same character.');
  if (patterns.some(p => p.type === 'date')) feedback.push('Avoid dates (years, birthdays).');
  return {
    input: s, length: s.length, uniqueChars: unique,
    shannonBitsPerChar: shannon, naiveBits: naive.bits, effectiveBits: eff,
    poolSize: naive.poolSize, score, percent, label,
