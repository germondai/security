...
export async function sha(input: string, alg: 'sha1' | 'sha256' | 'sha384' | 'sha512', salt?: string): Promise<string> {
  const data = salt ? salt + input : input;
  if (hasWebCrypto) {
    const subtle = (globalThis as { crypto: { subtle: SubtleCrypto } }).crypto.subtle;
    const webAlg = alg === 'sha1' ? 'SHA-1' : alg === 'sha256' ? 'SHA-256' : alg === 'sha384' ? 'SHA-384' : 'SHA-512';
...
export function shaSync(input: string, alg: 'sha1' | 'sha256' | 'sha384' | 'sha512', salt?: string): string {
  return nodeCreateHash(alg).update(salt ? salt + input : input, 'utf8').digest('hex');
}
