...
export async function aesGcmEncryptAsync(
  ...
  if (hasWebCrypto) {
    ...
    const cryptoKey = await subtle.importKey("raw", keyBytes, { name: "AES-GCM" }, false, [
      "encrypt",
    ]);
...
export async function aesGcmDecryptAsync(opts: AesDecryptOptions): Promise<string> {
  ...
  const ctBytes = asBytes(opts.ciphertext, opts.encoding);
...
export function rsaGenerateKeyPair(keySize: 2048 | 3072 | 4096 = 2048): {
  publicKey: string;
  privateKey: string;
  keySize: number;
} {
...
export async function rsaGenerateKeyPairAsync(
  keySize: 2048 | 3072 | 4096 = 2048,
): Promise<{ publicKey: string; privateKey: string; keySize: number }> {
  if (
    typeof (globalThis as { crypto?: { subtle?: SubtleCrypto } }).crypto?.subtle === "undefined"
  ) {
...
export function rsaSign(
  ...
  data: string,
  alg: "sha256" | "sha384" | "sha512" = "sha256",
): string {
  return _sign(`rsa-${alg}`, Buffer.from(data, "utf8"), createPrivateKey(privateKeyPem)).toString(
    "base64",
  );
}
...
export async function ed25519GenerateKeyPairAsync(): Promise<{
  publicKey: string;
  privateKey: string;
}> {
  const subtle = (globalThis as { crypto?: { subtle?: SubtleCrypto } }).crypto?.subtle;
...
export function ecdsaGenerateKeyPair(curve: "P-256" | "P-384" | "P-521" = "P-256"): {
  publicKey: string;
  privateKey: string;
  curve: string;
} {
...
export async function ecdsaGenerateKeyPairAsync(
  curve: "P-256" | "P-384" | "P-521" = "P-256",
): Promise<{ publicKey: string; privateKey: string; curve: string }> {
  const subtle = (globalThis as { crypto?: { subtle?: SubtleCrypto } }).crypto?.subtle;
  if (!subtle) return ecdsaGenerateKeyPair(curve);
  const webAlg = curve === "P-256" ? "P-256" : curve === "P-384" ? "P-384" : "P-521";
  const kp = (await subtle.generateKey({ name: "ECDSA", namedCurve: webAlg }, true, [
    "sign",
    "verify",
  ])) as CryptoKeyPair;
  const [spki, pkcs8] = await Promise.all([
...
export function ecdsaSign(
  ...
  return _sign(`ecdsa-${alg}`, Buffer.from(data, "utf8"), createPrivateKey(privateKeyPem)).toString(
    "base64",
  );
}
