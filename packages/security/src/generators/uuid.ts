...
function uuidV1(): string {
  ...
  b[7] = timeHi & 0xff;
  b[8]  = clockSeq[0] ?? 0; b[9] = clockSeq[1] ?? 0;
...
export function generateNanoId(size = 21, alphabet = NANOID_ALPH): string {
  ...
  for (let i = 0; i < size; i++) {
    let r: number;
    do { r = (secureRandomBytes(1)[0] ?? 0) & mask; } while (r >= alphabet.length);
...
export function generateKsuid(): string {
  ...
  full.set(tsBuf, 0); full.set(rand, 4);
  const digits: number[] = [];
  for (const byte of full) {
    let carry = byte;
    for (let i = 0; i < digits.length; i++) {
      carry += (digits[i] ?? 0) * 256;
      digits[i] = carry % 62; carry = Math.floor(carry / 62);
    }
...
export function generateUuids(count: number, opts: UuidOptions = { version: 'v4' }): string[] {
  if (!Number.isInteger(count) || count < 1 || count > 1000) throw new RangeError('count must be an integer in [1, 1000]');
