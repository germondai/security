...
import { rsaSign, ed25519Sign, ecdsaSign, rsaVerify, ed25519Verify, ecdsaVerify } from '@germondai/security';
...
export const signLeaf = defineCommand({
  ...
  args: {
    ...
    'private-key': { type: 'string', required: true, description: 'PEM-encoded private key (or @file or stdin).' },
    input:      { type: 'string', description: 'Data to sign (string, file, or stdin).' },
    alg:        { type: 'string', default: 'sha256', description: 'For rsa/ecdsa: sha256 | sha384 | sha512' },
...
export const verifyLeaf = defineCommand({
  ...
  args: {
    algorithm:  { type: 'string', required: true, description: 'rsa | ed25519 | ecdsa' },
    'public-key': { type: 'string', required: true, description: 'PEM-encoded public key (or @file or stdin).' },
  ...
  run({ args }) {
    try {
      ...
      const hashAlg = (args['alg'] ?? 'sha256') as 'sha256' | 'sha384' | 'sha512';
      const enc = (args['encoding'] as 'base64' | 'hex') ?? 'base64';
      const sig = enc === 'hex' ? Buffer.from(args['signature'] as string, 'hex').toString('base64') : args['signature'] as string;
      let valid: boolean;
      const algo = (args['algorithm'] ?? 'rsa') as string;
