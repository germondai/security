...
export const decodeCmd = defineCommand({
  meta: {
    name: "decode",
    description: "Decode base64/base32/base58/hex/binary/octal/decimal to text.",
  },
  args: {
    from: {
      type: "string",
    ...
    try {
      ...
      switch (from) {
        ...
        case "base64url":
          ...
          break;
        case "base32":
          bytes = fromBase32(text);
          break;
        case "base32hex":
          bytes = fromBase32Hex(text);
          break;
        case "base58":
          bytes = fromBase58(text);
          break;
        case "hex":
          bytes = fromHex(text);
          break;
        case "binary":
          bytes = fromBinary(text);
          break;
        case "octal":
          bytes = fromOctal(text);
          break;
        case "decimal":
          bytes = fromDecimal(text);
          break;
        default:
