...
export const uuidLeaf = defineCommand({
  meta: {
    name: "uuid",
    description: "Generate IDs: UUID v1/v3/v4/v5/v7, CUID, Nano ID, ULID, KSUID.",
  },
  args: {
  ...
  run({ args }) {
    try {
      ...
      if (kind === "cuid") out = Array.from({ length: n }, () => generateCuid(24));
        ...
        const opts: Parameters<typeof generateUuids>[1] = {
          version: (args.version ?? "v4") as UuidVersion,
        };
