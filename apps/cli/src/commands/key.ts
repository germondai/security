...
import { rsaGenerateKeyPair, ed25519GenerateKeyPair, ecdsaGenerateKeyPair } from '@germondai/security';
...
const convertLeaf = defineCommand({
  ...
  args: {
    ...
    input:      { type: 'string', required: true, description: 'PEM/DER (string) or JWK (string of JSON). File path or @file also works.' },
  },
  async run({ args }) {
    try {
