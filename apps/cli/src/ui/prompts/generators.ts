    ...
    if (version === "v3" || version === "v5") {
      ...
      if (ns === "cust") {
        const c = await p.text({
          ...
          validate: (x) =>
            /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(x))
              ? undefined
...
export async function secretWizard(): Promise<void> {
  ...
  const bytesStr = await p.text({
    ...
    validate: (v) => {
      const n = Number.parseInt(String(v), 10);
      return Number.isInteger(n) && n >= 8 && n <= 256
        ? undefined
        : "Must be an integer in [8, 256]";
    },
  });
  ...
  const prefix = await p.text({
    ...
    placeholder: "(none)",
    validate: (v) =>
      /^[a-zA-Z0-9_-]*$/.test(String(v)) ? undefined : "Only letters, digits, dash, underscore",
  });
