...
export async function hmacWizard(): Promise<void> {
  ...
  const verifyMode = await p.confirm({
    message: "Verify against an expected signature now?",
    initialValue: false,
  });
...
export async function keyPairWizard(): Promise<void> {
  ...
  try {
    ...
    else if (kind === "ecdsa") kp = ecdsaGenerateKeyPair(curve as "P-256" | "P-384" | "P-521");
    else kp = rsaGenerateKeyPair(rsaSize as 2048 | 3072 | 4096);
    s.stop("Done");
    p.note(
...
function decodeText(text: string, fmt: EncFmt): string {
  ...
  switch (fmt) {
    case "base64":
      ...
      break;
    case "base64url":
      b = fromBase64Url(text);
      break;
    case "base32":
      b = fromBase32(text);
      break;
    case "base32hex":
      b = fromBase32Hex(text);
      break;
    case "base58":
      b = fromBase58(text);
      break;
