...
export async function diceWizard(): Promise<void> {
  ...
  if (mode === "int") {
    const lo = await promptInt("Lower bound (inclusive)", {
      min: -1_000_000_000,
      max: 1_000_000_000,
    });
    if (p.isCancel(lo)) return p.cancel("Cancelled.");
    const hi = await promptInt("Upper bound (inclusive)", {
      min: lo as number,
      max: 1_000_000_000,
    });
...
export async function colorWizard(): Promise<void> {
  ...
  for (let i = 0; i < (count as number); i++) {
    ...
    if (format === "hex") lines.push(c.hex);
      ...
      lines.push(
        `${c.hex}  rgb(${c.rgb.r}, ${c.rgb.g}, ${c.rgb.b})  hsl(${c.hsl.h}, ${c.hsl.s}%, ${c.hsl.l}%)`,
      );
  }
  p.note(lines.join("\n"), "Colors");
  p.outro("Done.");
