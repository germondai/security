...
const tools = [
  {
    ...
    icon: "◈",
    name: "Password",
    desc: "Cryptographically random. All charset classes.",
  },
  {
    to: "/generators/passphrase",
    icon: "◇",
    name: "Passphrase",
    desc: "Diceware-style. 5 words ≈ 50 bits of entropy.",
  },
  {
    to: "/generators/secret",
    icon: "◉",
    name: "Secret / API key",
    desc: "Raw random bytes in hex / base64 / base64url.",
  },
  {
    to: "/generators/id",
    icon: "◍",
    name: "UUID v1-v7, CUID…",
    desc: "RFC 4122 + ULID, KSUID, Nano ID. Bulk up to 50.",
  },
  {
    to: "/analyzers/password-strength",
    icon: "▣",
    name: "Password strength",
    desc: "Entropy, patterns, time-to-crack scenarios.",
  },
  {
    to: "/hashes",
    icon: "⬡",
    name: "Hash (SHA)",
    desc: "SHA-1, SHA-256, SHA-384, SHA-512 + salt.",
  },
  {
    to: "/ciphers/aes",
    icon: "⬢",
    name: "AES-256-GCM",
    desc: "Authenticated encryption, passphrase-derived key.",
  },
  { to: "/ciphers/jwt", icon: "⬣", name: "JWT (HS256)", desc: "Sign and verify JSON Web Tokens." },
  {
    ...
    icon: "◆",
    name: "Key pairs",
    desc: "RSA, Ed25519, ECDSA. PEM output, in-browser.",
  },
  {
    to: "/encoders",
    icon: "⬚",
    name: "Universal converter",
    desc: "base64 / base32 / base58 / hex / binary / …",
  },
];
...
<template>
  <div class="space-y-16">
    <!-- Hero -->
    <section
      class="relative overflow-hidden rounded-2xl border border-[rgb(var(--border))] bg-gradient-to-br from-[rgb(var(--accent-soft))] via-[rgb(var(--bg))] to-[rgb(var(--bg))] p-8 sm:p-12"
    >
      <div
        class="absolute -right-12 -top-12 h-48 w-48 rounded-full bg-[rgb(var(--accent))] opacity-[0.08] blur-3xl"
      ></div>
      <div
        class="absolute -left-12 -bottom-12 h-48 w-48 rounded-full bg-[rgb(var(--ok))] opacity-[0.06] blur-3xl"
      ></div>
      ...
      <div class="relative space-y-6 text-center sm:text-left">
        ...
        <h1 class="text-4xl font-bold leading-tight tracking-tight sm:text-5xl">
          ...
          <span
            class="block bg-gradient-to-r from-[rgb(var(--accent))] to-[rgb(var(--ok))] bg-clip-text text-transparent"
          >
    ...
    <section class="space-y-4">
      <div class="flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        ...
        <span class="text-[0.85rem] text-[rgb(var(--fg-muted))] whitespace-nowrap"
          >{{ tools.length }}
          generators, analyzers, hashes, ciphers</span
        >
      </div>
      <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
        ...
        >
          <div class="flex items-center gap-2">
            ...
            <span
              aria-hidden="true"
              class="ml-auto text-[rgb(var(--fg-faint))] transition group-hover:translate-x-0.5 group-hover:text-[rgb(var(--accent))]"
              >→</span
            >
          </div>
    ...
    <section class="space-y-3">
      <div class="flex flex-col gap-1 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between">
        ...
        <span class="text-[0.85rem] text-[rgb(var(--fg-muted))] whitespace-nowrap"
          >every tool also lives on the command line</span
        >
      </div>
      <div class="card overflow-hidden">
        <div
          class="flex items-center gap-2 border-b border-[rgb(var(--border))] bg-[rgb(var(--bg-soft))] px-4 py-2.5 text-[0.78rem] text-[rgb(var(--fg-muted))]"
        >
        ...
        <pre
          class="m-0 overflow-x-auto bg-transparent p-4 text-[0.82rem] leading-relaxed text-[rgb(var(--fg))]"
        ><code>{{ cliExamples.join('\n') }}</code></pre>
      </div>
    </section>
    ...
    <section class="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
      <div class="card p-5">
        ...
        <p class="mt-1 text-[0.85rem] leading-relaxed text-[rgb(var(--fg-muted))]">
          No accounts, no history, no localStorage of secrets. Refresh the page and the analyzer
          forgets.
        </p>
      </div>
      <div class="card p-5">
        ...
        <p class="mt-1 text-[0.85rem] leading-relaxed text-[rgb(var(--fg-muted))]">
          Every result is yours to copy, pipe, or ignore. There is no "cloud sync" because there is
          no cloud.
        </p>
      </div>
      <div class="card p-5">
        ...
        <p class="mt-1 text-[0.85rem] leading-relaxed text-[rgb(var(--fg-muted))]">
          Both surfaces call the same <code>@germondai/security</code> package. No platform-specific
          code paths.
        </p>
      </div>
      <div class="card p-5">
        ...
        <p class="mt-1 text-[0.85rem] leading-relaxed text-[rgb(var(--fg-muted))]">
          Fast installs and tests in dev. The published <code>gsec</code> binary runs on any Node ≥
          20 LTS.
        </p>
      </div>
    </section>
