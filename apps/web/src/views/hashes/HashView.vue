...
watch([input, alg, salt, useSalt], () => {
  if (input.value) compute();
  else {
    output.value = "";
    isSalted.value = false;
  }
});
...
const cli = computed(() => {
  const parts = [
    "bun cli hash sha",
    `--algorithm ${alg.value}`,
    `--input "${input.value.replace(/"/g, '\\"').slice(0, 40)}"`,
  ];
  ...
  <article class="space-y-6">
    <header class="space-y-1">
      ...
      <p class="text-[0.9rem] text-[rgb(var(--fg-muted))]">
        SHA-1/256/384/512. WebCrypto in browser, node:crypto in CLI. Optional salt.
      </p>
    </header>

    <div class="card p-5 space-y-4">
      <div>
        <label for="hash-input" class="field-label">Input text</label>
        <textarea
          id="hash-input"
          v-model="input"
          rows="4"
      ...
      <div>
        ...
        <div class="tab-bar">
          <button
            v-for="a in (['sha1','sha256','sha384','sha512'] as const)"
            :key="a"
            type="button"
            @click="alg = a"
            :aria-pressed="alg === a"
            :class="{ 'is-active': alg === a }"
          >
            {{ a.toUpperCase() }}
          </button>
        </div>
      ...
      <div>
        <label class="flex items-center gap-3 cursor-pointer text-[0.92rem] select-none">
          <span class="switch" :class="{ 'is-on': useSalt }"
            ><input type="checkbox" v-model="useSalt" class="sr-only">
      ...
      <div>
        <p class="field-label">
          ...
          <span v-if="pending" class="ml-2 text-[0.75rem] text-[rgb(var(--fg-muted))]"
            >computing…</span
          >
        </p>
        <div class="flex items-stretch gap-2">
          <code
            class="flex-1 break-all rounded-md border border-[rgb(var(--border))] bg-[rgb(var(--bg))] px-3 py-2 font-mono text-[0.85rem]"
            >{{ output || '—' }}</code
          >
          <CopyButton :value="output" />
        </div>
        <p v-if="output" class="text-[0.75rem] text-[rgb(var(--fg-muted))] mt-1">
          {{ bits }}
          bits · {{ output.length / 2 }} bytes
        </p>
        <p v-if="errorMsg" class="text-[0.8rem] text-[rgb(var(--danger))] mt-1" role="alert">
          // {{ errorMsg }}
        </p>
      </div>
