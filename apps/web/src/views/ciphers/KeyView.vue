...
import {
  ecdsaGenerateKeyPairAsync,
  ed25519GenerateKeyPairAsync,
  rsaGenerateKeyPairAsync,
  ...
  <article class="space-y-6">
    <header class="space-y-2">
      ...
      <p class="text-[0.9rem] text-[rgb(var(--fg-muted))]">
        Generate asymmetric key pairs in PEM format (SPKI public, PKCS8 private). Click
        <strong>Generate</strong>
        to start — RSA 4096 takes a few seconds.
      </p>
    </header>

    <div class="card p-5 space-y-5">
      <div>
        <p class="field-label">Algorithm</p>
        <div class="tab-bar">
          <button
            v-for="k in (['rsa','ed25519','ecdsa'] as const)"
            :key="k"
            type="button"
            @click="kind = k"
            :aria-pressed="kind === k"
      ...
      <div v-if="kind === 'rsa'">
        <p class="field-label">Modulus size</p>
        <div class="tab-bar">
          <button
            v-for="s in [2048, 3072, 4096]"
            :key="s"
            type="button"
            @click="rsaSize = s"
            :aria-pressed="rsaSize === s"
            :class="{ 'is-active': rsaSize === s }"
          >
            {{ s }}
          </button>
        </div>
        <p class="mt-2 text-[0.78rem] text-[rgb(var(--fg-muted))]">
          3072 takes ~1-2s; 4096 takes ~5-10s on a modern laptop.
        </p>
      </div>

      <div v-else-if="kind === 'ecdsa'">
        <p class="field-label">Curve</p>
        <div class="tab-bar">
          <button
            v-for="c in (['P-256','P-384','P-521'] as const)"
            :key="c"
            type="button"
            @click="curve = c"
            :aria-pressed="curve === c"
            :class="{ 'is-active': curve === c }"
          >
            {{ c }}
          </button>
        </div>
      ...
      <button type="button" class="btn btn-accent w-full" :disabled="loading" @click="regenerate">
      ...
      <div v-if="result" class="space-y-3">
        <div>
          ...
          <div class="flex items-stretch gap-2">
            <pre
              class="pre-block flex-1 whitespace-pre overflow-x-auto"
            >{{ result.publicKey }}</pre>
            <CopyButton :value="result.publicKey" />
          </div>
        </div>
        <div>
          <p class="field-label flex items-center gap-2">
            Private key
