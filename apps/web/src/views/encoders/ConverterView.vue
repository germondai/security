  ...
  try {
    if (mode.value === "encode") {
      ...
      switch (fromEnc.value) {
        case "base64":
          bytes = fromBase64(input.value);
          break;
        case "base64url":
          bytes = fromBase64Url(input.value);
          break;
        case "base32":
          bytes = fromBase32(input.value);
          break;
        case "base32hex":
          bytes = fromBase32Hex(input.value);
          break;
        case "base58":
          bytes = fromBase58(input.value);
          break;
        case "hex":
          bytes = fromHex(input.value);
          break;
        case "binary":
          bytes = fromBinary(input.value);
          break;
        case "octal":
          bytes = fromOctal(input.value);
          break;
        case "decimal":
          bytes = fromDecimal(input.value);
          break;
      }
    ...
    <div class="card p-5 space-y-5">
      ...
      <div class="flex items-center gap-3">
        <div class="flex-1 text-center">
          <span v-if="mode === 'encode'" class="text-[0.9rem] text-[rgb(var(--fg-muted))]"
            >text</span
          >
      ...
      <div class="grid gap-4 md:grid-cols-2">
        <div>
          <label for="converter-input" class="field-label">Input</label>
          <textarea
            id="converter-input"
            v-model="input"
            rows="10"
