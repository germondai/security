...
const cli = computed(
  () => `bun cli analyze --input "${input.value.replace(/"/g, '\\"').slice(0, 60) || "…"}"`,
);
...
<template>
  <article class="space-y-6">
    <header class="space-y-2 flex items-start justify-between gap-4 flex-wrap">
      ...
      <button type="button" class="btn btn-accent w-full" @click="generateStrong">
    ...
    <div class="card p-5 space-y-4">
      <div>
        <label for="strength-input" class="field-label">Candidate password</label>
        <input
          id="strength-input"
          type="text"
          v-model="input"
        ...
        >
        <div
          class="mt-2 flex items-center justify-between text-[0.78rem] text-[rgb(var(--fg-muted))]"
        >
          <span
            >Try:
            <button
              class="underline hover:text-[rgb(var(--accent))]"
              @click="tryExample"
              type="button"
            >
              {{ EXAMPLE }}
      ...
      <div v-if="result" class="space-y-4">
        <div class="space-y-1.5">
          <div class="flex items-center justify-between text-[0.85rem]">
            ...
            <span class="font-bold" :style="{ color: scoreColor(result.score) }"
              >{{ result.label }}
              ({{ result.score }}/4)</span
            >
          </div>
          <div class="flex gap-1.5">
            <div
              v-for="i in 5"
              :key="i"
              class="h-2 flex-1 rounded-full border"
        ...
        <div class="grid grid-cols-2 gap-3 text-[0.85rem] sm:grid-cols-4">
          <div>
            <div class="text-[0.7rem] uppercase tracking-wider text-[rgb(var(--fg-muted))]">
              Length
            </div>
          ...
          <div>
            ...
            <div class="text-lg font-bold">{{ result.uniqueChars }}</div>
          </div>
          <div>
            <div class="text-[0.7rem] uppercase tracking-wider text-[rgb(var(--fg-muted))]">
              Naive bits
            </div>
            <div class="text-lg font-bold">{{ result.naiveBits.toFixed(1) }}</div>
          </div>
          <div>
            <div class="text-[0.7rem] uppercase tracking-wider text-[rgb(var(--fg-muted))]">
              Effective bits
            </div>
            <div class="text-lg font-bold" :style="{ color: scoreColor(result.score) }">
              {{ result.effectiveBits.toFixed(1) }}
            </div>
          </div>
        </div>
        ...
        <div>
              ...
              <div class="flex items-start justify-between gap-3">
                <div class="font-mono text-[0.85rem] min-w-0">
                  <template v-if="scenarioParts(ct.scenario).tag">
                    ...
                    <div class="text-[0.72rem] text-[rgb(var(--fg-faint))]">
                      {{ scenarioParts(ct.scenario).tag }}
                    </div>
                  </template>
                  <template v-else>{{ ct.scenario }}</template>
                </div>
                <div
                  class="shrink-0 text-right text-[0.78rem] tabular-nums text-[rgb(var(--fg-muted))]"
                >
                  <div>
                    {{ ct.guessesPerSecond.toExponential(0) }}
                    <span class="text-[0.65rem]"> /s</span>
                  </div>
                </div>
              </div>
              <div class="mt-2 grid grid-cols-2 gap-3 text-[0.85rem]">
                <div>
                  <div class="text-[0.62rem] uppercase tracking-wider text-[rgb(var(--fg-faint))]">
                    Average
                  </div>
          ...
          <p class="text-[0.7rem] text-[rgb(var(--fg-muted))] leading-relaxed">
            Values beyond 10¹⁵ seconds use scientific notation; "forever" means longer than the heat
      ...
      <div v-else class="space-y-3 border-t pt-4 text-[0.85rem] text-[rgb(var(--fg-muted))]">
        <p class="text-[0.95rem] font-semibold text-[rgb(var(--fg))]">
          Try one of these (or click <strong>⚡ Generate strong password</strong> above):
        </p>
        <ul class="space-y-1.5 font-mono text-[0.8rem]">
          <li>
            <button
              ...
              class="underline hover:text-[rgb(var(--danger))]"
            >
              password
            </button>
            — <span class="text-[rgb(var(--danger))]">the classic</span>
          </li>
          <li>
            <button
              type="button"
              @click="input = 'P@ssw0rd!'"
              class="underline hover:text-[rgb(var(--danger))]"
            >
              P@ssw0rd!
            </button>
            — <span class="text-[rgb(var(--danger))]">looks strong, is not</span>
          </li>
          <li>
            <button
              type="button"
              @click="input = 'qwerty12345'"
              class="underline hover:text-[rgb(var(--danger))]"
            >
              qwerty12345
            </button>
            — <span class="text-[rgb(var(--danger))]">keyboard walk + sequence</span>
          </li>
          <li>
            <button
              type="button"
            ...
            — <span class="text-[rgb(var(--danger))]">common dictionary word</span>
          </li>
          <li>
            <button
              type="button"
              @click="input = 'Q3v#7mZ!9xP@kL$5nR&Yd8Wq2'"
              class="underline hover:text-[rgb(var(--ok))]"
            >
              Q3v#7mZ!9xP@kL$5nR&Yd8Wq2
            </button>
            — <span class="text-[rgb(var(--ok))]">this should be strong</span>
          </li>
        </ul>
      </div>
    </div>
