...
const outputs = computed<string[]>(() => {
  void seed.value;
  try {
    return Array.from({ length: count.value }, () => generatePassword(options.value));
  } catch {
    return [];
  }
});
  ...
  <article class="space-y-6">
    <header class="space-y-1">
      ...
      <p class="text-[0.9rem] text-[rgb(var(--fg-muted))]">
        Cryptographically random, generated locally with <code>node:crypto</code>.
      </p>
    </header>
    ...
    <div class="card p-5 space-y-5">
      <div>
        ...
        <div class="tab-bar">
          <button
            ...
            :class="{ 'is-active': activePreset === 'memorable' }"
          >
            Memorable
          </button>
          <button
            ...
            :class="{ 'is-active': activePreset === 'standard' }"
          >
            Standard
          </button>
          <button
            ...
            :class="{ 'is-active': activePreset === 'maximum' }"
          >
            Maximum
          </button>
        </div>
      ...
      <div class="space-y-2">
        <div v-for="(pwd, idx) in outputs" :key="idx" class="flex items-stretch gap-2 min-w-0">
          <div
            class="min-w-0 flex-1 flex items-center scroll-x-hidden whitespace-nowrap rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))] px-3 h-[2.4rem] font-mono text-[0.95rem]"
          >
            {{ pwd || '—' }}
          </div>
          <button
      ...
      <div class="grid gap-4 sm:grid-cols-2">
        <div>
          <label for="pwd-length" class="field-label"
            >Length: <span class="text-[rgb(var(--accent))]">{{ length }}</span></label
          >
          <input
            id="pwd-length"
            type="range"
        ...
        <div>
          <label for="pwd-count" class="field-label"
      ...
      <div class="grid grid-cols-1 gap-3 text-[0.9rem] sm:grid-cols-2 lg:grid-cols-3">
        <label class="flex items-center gap-3 cursor-pointer select-none">
          <span class="switch" :class="{ 'is-on': lower }"
            ><input type="checkbox" v-model="lower" class="sr-only">
        ...
        <label class="flex items-center gap-3 cursor-pointer select-none">
          <span class="switch" :class="{ 'is-on': digits }"
            ><input type="checkbox" v-model="digits" class="sr-only">
            <span class="switch-knob"></span></span
          >
          <span>Digits (0–9)</span>
        </label>
        <label class="flex items-center gap-3 cursor-pointer select-none">
          <span class="switch" :class="{ 'is-on': symbols }"
            ><input type="checkbox" v-model="symbols" class="sr-only">
            <span class="switch-knob"></span></span
          >
          <span>Symbols (!@#…)</span>
        </label>
        <label class="flex items-center gap-3 cursor-pointer select-none">
          <span class="switch" :class="{ 'is-on': excludeAmbiguous }"
            ><input type="checkbox" v-model="excludeAmbiguous" class="sr-only">
            <span class="switch-knob"></span></span
          >
          <span>Exclude ambiguous (0Oo1lI|`')</span>
        </label>
        <label class="flex items-center gap-3 cursor-pointer select-none">
          <span class="switch" :class="{ 'is-on': requireEach }"
            ><input type="checkbox" v-model="requireEach" class="sr-only">
            <span class="switch-knob"></span></span
          >
