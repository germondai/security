  ...
  <article class="space-y-6">
    <header class="space-y-2">
      ...
      <p class="text-[0.9rem] text-[rgb(var(--fg-muted))]">
        Diceware-style. 8 words ≈ 80 bits. Up to 32 words.
      </p>
    </header>
    ...
    <div class="card p-5 space-y-5">
      <div class="flex items-stretch gap-2 min-w-0">
        <div
          class="min-w-0 flex-1 flex items-center scroll-x-hidden whitespace-nowrap rounded-xl border border-[rgb(var(--border))] bg-[rgb(var(--bg))] px-3 h-[2.4rem] font-mono text-[1.05rem] tracking-wide"
        >
          {{ phrase }}
        </div>
      ...
      <p class="text-[0.85rem] text-[rgb(var(--fg-muted))]">
        Estimated entropy:
        <span class="font-mono text-[rgb(var(--accent))]">~{{ bits.toFixed(0) }} bits</span>
        <span v-if="bits < 50" class="ml-2 text-[rgb(var(--danger))]"
          >— consider at least 5 words</span
        >
      ...
      <div class="space-y-4">
        <div class="flex flex-col">
          <label for="passphrase-words" class="field-label"
            >Words: <span class="text-[rgb(var(--accent))]">{{ words }}</span></label
          >
          <input
            id="passphrase-words"
            type="range"
        ...
        <div class="flex flex-col">
          <label for="passphrase-sep" class="field-label">Separator</label>
          <input
            id="passphrase-sep"
            type="text"
            v-model="separator"
      ...
      <div
        class="space-y-3 text-[0.92rem] sm:grid sm:grid-cols-2 sm:gap-x-6 sm:gap-y-2 sm:space-y-0"
      >
