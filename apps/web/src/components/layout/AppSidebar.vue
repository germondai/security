...
const groups: Group[] = [
  ...
  },
  {
    label: 'Analyzers',
    items: [
      { to: '/analyzers/password-strength', label: 'Password strength' },
    ],
  },
  {
    label: 'Hashes',
  ...
  <aside class="flex h-full w-72 flex-col gap-5 border-r border-[rgb(var(--border))] bg-[rgb(var(--bg-soft))] p-4">
    ...
    <nav class="flex flex-1 flex-col gap-4 overflow-y-auto">
      <div v-for="g in groups" :key="g.label" class="flex flex-col gap-1">
        <p class="px-2 text-[0.65rem] font-semibold uppercase tracking-[0.14em] text-[rgb(var(--fg-faint))]">{{ g.label }}</p>
        <RouterLink
          v-for="i in g.items"
          :key="i.label"
          :to="i.to"
